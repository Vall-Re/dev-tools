'use client';

import { useState } from 'react';

export default function UrlConverter() {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [error, setError] = useState('');

  const encodeUrl = () => {
    try {
      if (!input) return;
      setOutput(encodeURIComponent(input));
      setError('');
    } catch (err: unknown) {
      setError('Failed to encode URL: ' + (err as Error).message);
      setOutput('');
    }
  };

  const decodeUrl = () => {
    try {
      if (!input) return;
      setOutput(decodeURIComponent(input));
      setError('');
    } catch (err: unknown) {
      setError('Invalid URL encoding format.');
      setOutput('');
    }
  };

  return (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium mb-2">Input URL or Text</label>
        <textarea
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Enter plain text or URL encoded string here..."
          className="w-full h-32 p-3 border rounded-lg font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>

      <div className="flex gap-2">
        <button
          onClick={encodeUrl}
          className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition"
        >
          Encode
        </button>
        <button
          onClick={decodeUrl}
          className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition"
        >
          Decode
        </button>
      </div>

      {error && (
        <div className="p-3 border border-red-300 bg-red-50 text-red-700 rounded-lg text-sm font-mono">
          <strong>Error:</strong> {error}
        </div>
      )}

      {output && (
        <div>
          <label className="block text-sm font-medium mb-2">Result</label>
          <pre className="w-full p-3 border rounded-lg bg-gray-900 text-green-400 font-mono text-sm overflow-x-auto whitespace-pre-wrap break-all">
            {output}
          </pre>
        </div>
      )}
    </div>
  );
}